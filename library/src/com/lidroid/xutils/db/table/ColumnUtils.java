...
public class ColumnUtils {
    ...
    public static Method getColumnGetMethod(Class<?> entityType, Field field) {
        ...
        if (field.getType() == boolean.class) {//boolean属性值
            getMethod = getBooleanColumnGetMethod(entityType, fieldName);
        }
        if (getMethod == null) {
            String methodName = "get" + fieldName.substring(0, 1).toUpperCase() + fieldName.substring(1);//构造GET方法语句
            try {
                getMethod = entityType.getDeclaredMethod(methodName);//获得Method
