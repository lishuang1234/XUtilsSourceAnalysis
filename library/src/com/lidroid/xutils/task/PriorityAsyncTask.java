...
public abstract class PriorityAsyncTask<Params, Progress, Result> implements TaskHandler {
    ...
    private Result postResult(Result result) {
        @SuppressWarnings("unchecked") Message message = sHandler.obtainMessage
     ...
     * @see #execute(Object[])
     * 在线程池中执行请求
     * 1.如果线程池中已经在执行任务，抛出异常
     * 2.主线程中执行onPreExecute();执行前的准备
     * 3.将HttpRequest参数添加到Callable对象中，以此参数执行请求
     * 4.新建Runnable对象并存储FutureTask引用，以及优先级引用
     * 5.在线程池中执行Runnable对象，代理执行FutureTask中的run方法，即执行Callable对象。
     *
     */
    public final PriorityAsyncTask<Params, Progress, Result> executeOnExecutor(Executor exec,
        ...
        onPreExecute();
        mWorker.mParams = params;//添加HttpRequest
        exec.execute(new PriorityRunnable(priority, mFuture));//生成对象代理完成FutureTask
        return this;
    }
    ...
    protected final void publishProgress(Progress... values) {
        if (!isCancelled()) {
            sHandler.obtainMessage(MESSAGE_POST_PROGRESS, new AsyncTaskResult<Progress>(this,
                    values)).sendToTarget();
        }
    }
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case MESSAGE_POST_RESULT://任务结束
                    ...
                    result.mTask.finish(result.mData[0]);//调用PriorityAsyncTask引用结束任务
                    break;
                case MESSAGE_POST_PROGRESS://任务进度
                    result.mTask.onProgressUpdate(result.mData);//调用PriorityAsyncTask引用发布进度
                    break;
            }
        }
    }
