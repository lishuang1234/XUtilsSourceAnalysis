...
public class HttpRequest extends HttpRequestBase implements HttpEntityEnclosingRequest {
     ...
     * 2.设置请求头信息
     * 3.设置请求字符串参数信息
     * 4.设置请求HttpEntity，主要用于上传文件
     */
    public void setRequestParams(RequestParams param, RequestCallBackHandler callBackHandler) {
        if (param != null) {
            if (uriCharset == null) {
                uriCharset = Charset.forName(param.getCharset());//获取编码方式
            }
            List<RequestParams.HeaderItem> headerItems = param.getHeaders();//请求头
            if (headerItems != null) {
                for (RequestParams.HeaderItem headerItem : headerItems) {//设置头
                    if (headerItem.overwrite) {
                        this.setHeader(headerItem.header);
                    } else {
                        this.addHeader(headerItem.header);
            ...
            this.addQueryStringParams(param.getQueryStringParams());//设置请求字符串参数
            HttpEntity entity = param.getEntity();//获取请求HttpEntity
            if (entity != null) {
                if (entity instanceof UploadEntity) {
                    ((UploadEntity) entity).setCallBackHandler(callBackHandler);//如果是上传实体设置回调
                }
                this.setEntity(entity);//设置HttpEntity
            }
        }
    }
